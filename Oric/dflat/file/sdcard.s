...
; Eerror routine if Arduino does not respond in time
; E.g not connected, or could not find file (for read, write, delete)
; This is only called from sd_ready (not a jsr) and causes BRK if C=1 on the stack
sd_timeout
	plp
	bcs sd_timeout_brk
	sec							; C=1 means error
...
;* If returns (i.e. no BRK) then C=1 means timeout error else ok
sd_ready_sec
	sec
sd_ready
	php
...
sd_ready_skipx
	...
	plp
	clc							; C=0 means ok
	; Fall through to delay routine once ready check is ok
...
sd_delete
	...
	jsr sd_ready_sec		; Check SD card is ready else error
