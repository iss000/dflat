...
df_rt_init_filename
	...
	lda io_default					; Unless no arduino detected
	bmi df_rt_sd_connected
	ldx #0							; In which case assume tape i.e. device 0
df_rt_sd_connected
	ldy #1
	lda (df_tmpptrc),y
	...
	bne df_rt_try_tape
	lda io_default					; But if no sd card then error
	bpl df_rt_file_errc
	bmi df_rt_do_fname
df_rt_try_tape
...
df_rt_loadline
	...
	; df_tokbuff contains the last line loaded, use this
	; to decide it autostart is needed or not
	lda df_tokbuff+4		; Check the first token in the token buffer
	bpl df_rt_no_autostart	; If less than 0x80 then not a keyword
	cmp #DFRT_DEF			; Must be less than the def keyword
	bcs df_rt_no_autostart
	jmp df_pg_runcommand
df_rt_no_autostart
	; Back to the ready prompt when done (resets the stack too)
	jmp df_pg_dflat
