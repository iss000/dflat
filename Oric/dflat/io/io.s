...
io_init
	if DFLATLOCI
	jsr lc_init_path	; Initialise the LOCI sd card path
	lda #1				; Default = KB/VDP, no Arduino sd card supported
	else
	jsr f_init_default	; Determine if Arduino sd card connected or not (returnx 0x00 or 0x80)
	ora #1				; Default = KB/VDP, MSB=Arduino card connected
	endif
	sta io_default
